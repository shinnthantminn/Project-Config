...
class RecentTutorialScreen extends React.PureComponent {
  ...
  load = (forceRefresh) => {
    ...
    this.digiedModule
      .getTutorialsByIds({
        page: this.page,
        ids: this.recent_tutorial_ids.join(","),
      })
      .then((response) => {
        ...
        _.each(response.data.data, (data, index) => {
          ...
          if (data.tutorial_image) {
            tutorial.tutorial_image =
              CommonConstants.storage_endpoint + "/" + data.tutorial_image;
          }

          if (
            i18n.language != "en" &&
            data.tutorial_title_l10n &&
            data.tutorial_title_l10n[i18n.language]
          ) {
          ...
          if (
            i18n.language != "en" &&
            data.tutorial_description_l10n &&
            data.tutorial_description_l10n[i18n.language]
          ) {
          ...
          if (data.tutorial_video_file) {
            ...
            tutorial.video_url =
              ...
              "/digied-module/tutorials/" +
              data.tutorial_hash_ref +
              ".m3u8";
          }

          if (data.tutorial_summary_pdf_file) {
            tutorial.tutorial_summary_pdf_file =
              CommonConstants.storage_endpoint +
              "/" +
              data.tutorial_summary_pdf_file;
          }
          ...
          if (data.chapter) {
            ...
            if (
              i18n.language != "en" &&
              data.chapter.chapter_no_l10n &&
              data.chapter.chapter_no_l10n[i18n.language]
            ) {
            ...
            if (
              i18n.language != "en" &&
              data.chapter.chapter_title_l10n &&
              data.chapter.chapter_title_l10n[i18n.language]
            ) {
              tutorial.chapter_title =
                data.chapter.chapter_title_l10n[i18n.language];
            ...
            if (data.chapter.subject) {
              ...
              if (
                i18n.language != "en" &&
                data.chapter.subject.subject_name_l10n &&
                data.chapter.subject.subject_name_l10n[i18n.language]
              ) {
                tutorial.subject_title =
                  data.chapter.subject.subject_name_l10n[i18n.language];
                tutorial.subject_name_l10n =
                  data.chapter.subject.subject_name_l10n;
              }

              if (data.chapter.subject.course) {
                tutorial.course_title = data.chapter.subject.course.course_name;
                tutorial.course_title_en =
                  data.chapter.subject.course.course_name;

                if (
                  i18n.language != "en" &&
                  data.chapter.subject.course.course_name_l10n &&
                  data.chapter.subject.course.course_name_l10n[i18n.language]
                ) {
                  tutorial.course_title =
                    data.chapter.subject.course.course_name_l10n[i18n.language];
                  tutorial.course_name_l10n =
                    data.chapter.subject.course.course_name_l10n;
                }

                if (data.chapter.subject.course.course_category) {
                  tutorial.course_category_title =
                    data.chapter.subject.course.course_category.course_category_name;
                  tutorial.course_category_title_en =
                    data.chapter.subject.course.course_category.course_category_name;

                  if (
                    i18n.language != "en" &&
                    data.chapter.subject.course.course_category
                      .course_category_name_l10n &&
                    data.chapter.subject.course.course_category
                      .course_category_name_l10n[i18n.language]
                  ) {
                    tutorial.course_category_title =
                      data.chapter.subject.course.course_category.course_category_name_l10n[
                        i18n.language
                      ];
                    tutorial.course_category_name_l10n =
                      data.chapter.subject.course.course_category.course_category_name_l10n;
                  }
                }
              }
          ...
          tutorial.favourited_info = _.find(
            response.data.favourited_tutorials,
            (favourited_tutorial) => {
              return favourited_tutorial.tutorial_id == data.id;
            }
          );

          tutorial.total_favourited_count =
            response.data.total_favourited_count;
          tutorial.total_favourited_count_display =
            response.data.total_favourited_count_display;
          ...
          <Grid>
              ...
              <Box>
                {i18n.language == "en" && (
                  <Text
                    ...
                    numberOfLines={2}
                  >
                    {item.course_title} , {item.subject_title}
                  </Text>
                )}
            ...
            >
              ...
              {item.is_favourting_in_progress == false &&
                !item.favourited_info && (
                  <TouchableOpacity
                    onPress={this._onClickedFavourite(item)}
                    style={{
                      width: 30,
                      height: 30,
                      justifyContent: "flex-start",
              ...
              {item.is_favourting_in_progress == false &&
                item.favourited_info &&
                item.favourited_info._id && (
                  <TouchableOpacity
                    onPress={this._onClickedUnfavourite(item)}
                    style={{
                      width: 30,
                      height: 30,
                      justifyContent: "flex-start",
  ...
  _onClickedFavourite = (item) => () => {
    ...
    this.digiedModule
      .favouriteTutorial({ tutorial_id: item.id })
      .then((response) => {
        item.is_favourting_in_progress = false;
        ...
        item.total_favourited_count_display =
          response.data.total_favourited_count_display;
  ...
  _onClickedUnfavourite = (item) => () => {
    ...
    this.digiedModule
      .unfavouriteTutorial({ tutorial_id: item.id })
      .then((response) => {
        item.is_favourting_in_progress = false;
...
export default withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(RecentTutorialScreen)
);
