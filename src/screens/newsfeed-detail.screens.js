...
class NewsfeedDetailScreen extends React.PureComponent {
  ...
  load = (forceRefresh) => {
    ...
    this.digiedModule
      .getNewsfeedComments({
        ...
        page: this.page,
        per_page: 10,
        include_sub_comments_per_page: 5,
      })
      .then((response) => {
        _.each(response.data.data, (data, index) => {
          ...
          comment.created_by_user = _.find(
            response.data.created_by_users,
            (created_by_user) => {
          ...
          if (
            ...
            comment.created_by_user.profile_image
          ) {
            comment.created_by_user = _.clone(comment.created_by_user);
            comment.created_by_user.profile_image =
              CommonConstants.storage_endpoint +
              "/" +
              comment.created_by_user.profile_image;
          }

          let sub_comment_total = _.find(
            response.data.sub_comment_total_counts,
            (sub_comment_total_count) => {
              return sub_comment_total_count.parent_comment_id == comment.id;
            }
          );

          if (sub_comment_total) {
            let sub_comments = _.filter(
            ...
            _.each(sub_comments, (sub_comment) => {
              ...
              subcomment.created_by_user = _.find(
                response.data.created_by_users,
                (created_by_user) => {
              ...
              if (
                subcomment.created_by_user &&
                subcomment.created_by_user.profile_image
              ) {
                subcomment.created_by_user = _.clone(
                  subcomment.created_by_user
                );
                subcomment.created_by_user.profile_image =
                  CommonConstants.storage_endpoint +
  ...
  renderNewsfeedComment = ({ item, index }) => {
    ...
    const finalWidth =
      width -
      (COMMON_STYLE.PADDING * 2 + 10 + (item.parent_comment_id ? 10 : 0));
      ...
      <Fragment>
        <Box
          style={{
            ...
            marginLeft:
              COMMON_STYLE.PADDING + 10 + (item.parent_comment_id ? 10 : 0),
        ...
        >
          <Grid>
            <Row>
              ...
              >
                {item.commented_by_type == 1 &&
                  item.created_by_user &&
                  item.created_by_user.profile_image && (
                    <Box
                      style={{
              ...
              <Col>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <TouchableOpacity
                        onPress={this._onClickedNewsfeedCommentLike(item)}
                      >
                        <HStack space={1}>
                          ...
                          {item.is_liking_in_progress == false && (
                            <FontAwesomeIcon
                              icon={item.has_liked ? faHeart : farHeart}
                              size={15}
                              style={{
                                color: COLORS.GREEN,
                              }}
                          ...
                          <Text
                            ...
                            numberOfLines={1}
                          >
                            {item.total_liked_count} {t("Likes")}
                          </Text>
                        </HStack>
                      </TouchableOpacity>
                    </Col>
                    ...
                    >
                      {!item.parent_comment_id && (
                        <TouchableOpacity
                          onPress={this._onClickedNewsfeedCommentReply(item)}
                        >
                          <HStack space={1}>
                            <FontAwesomeIcon
                              icon={faComment}
        ...
        {item.sub_comments && item.sub_comments.length > 0 && (
          <FlatList
            keyExtractor={(item, index) =>
              "newsfeeds_subcomment_" + index.toString()
            }
            ...
            ListFooterComponent={
              item.sub_comment_has_next && (
                ...
                >
                  ...
                  {item.is_sub_comment_loading != true && (
                    <TouchableOpacity
                      onPress={this._onLoadMoreSubcomments(item)}
                    >
                      <Text
                        style={[
                          COMMON_STYLES[i18n.language].regular,
  ...
  _onVideoError = (e) => {
    if (this.hasErrorDisplay == false) {
      ...
      this.modalAlert = {
        ...
        description: t(
          "Unable to play the tutorial video. Please contact support team"
        ),
      };
      this.setState({
        forceRender: !this.state.forceRender,
  ...
  _orientationDidChange = (orientation) => {
    ...
    if (this.videoPlayer.player != false) {
      this.timeBeforeScreenOrientationChange =
        this.videoPlayer.player.calculateTimeFromSeekerPosition();
    }
  ...
  render() {
    ...
    const imageWidth =
      this.newsfeed_detail && this.newsfeed_detail.image
        ? finalWidth - COMMON_STYLE.PADDING * 2
        : 0;
      ...
      <NativeBaseProvider>
        ...
        >
          ...
          >
            ...
            {isConnected && loading == true && <Spinner color={COLORS.THEME} />}
              ...
              {isConnected &&
                ...
                this.orientation == "PORTRAIT" && (
                  <Fragment>
                    <FlatList
                      keyExtractor={(item, index) =>
                        "newsfeeds_comment_" + index.toString()
                      }
                      ...
                      ListHeaderComponent={
                        ...
                        >
                                ...
                                >
                                  ...
                                  >
                                    <Text
                                      ...
                                      numberOfLines={1}
                                    >
                                      {this.newsfeed_detail.created_at_display}
                                    </Text>
                                  </Box>
                                </Col>
                          ...
                          >
                            ...
                            >
                              {this.newsfeed_detail.content}
                            </Text>
                          </Box>

                          {this.newsfeed_detail.video_url == "" &&
                            this.newsfeed_detail.image != "" && (
                              <Row
                                style={{
                                  marginTop: COMMON_STYLE.PADDING,
                                }}
                                ...
                                >
                                  <VideoPlayer
                                    ...
                                    onLoad={() => {
                                      if (
                                        this
                                          .timeBeforeScreenOrientationChange >=
                                        0
                                      ) {
                                  ...
                                  {this.videoPlayer.paused == true &&
                                    this.videoPlayer.showPlayIcon == true && (
                                      ...
                                      >
                                        <TouchableOpacity
                                          onPress={this._onClickedPlayVideo}
                                        >
                                          <Image
                                            source={play_icon}
                                            style={{
                          ...
                          >
                                ...
                                >
                                  <HStack space={5}>
                                    ...
                                    <Text
                                      ...
                                      numberOfLines={1}
                                    >
                                      {this.newsfeed_detail.total_shared_count}{" "}
                                      {t("Shares")}
                                    </Text>
                                  </HStack>
                                </Col>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <TouchableOpacity
                                    onPress={this._onClickedNewsfeedLike}
                                  >
                                    <HStack space={1}>
                                      {this.newsfeed_detail
                                        .is_liking_in_progress == true && (
                                        <Spinner
                                          color={COLORS.GREEN}
                                          style={{
                                            transform: [
                                      ...
                                      {this.newsfeed_detail
                                        ...
                                        <FontAwesomeIcon
                                          icon={
                                            this.newsfeed_detail.has_liked
                                              ? faHeart
                                              : farHeart
                                          }
                                          size={15}
                                      ...
                                      <Text
                                        ...
                                        numberOfLines={1}
                                      >
                                        {this.newsfeed_detail.total_liked_count}{" "}
                                        {t("Likes")}
                                      </Text>
                                    </HStack>
                                  </TouchableOpacity>
                                ...
                                >
                                  <TouchableOpacity
                                    onPress={this._onClickedNewsfeedComment()}
                                  >
                                    <HStack space={1}>
                                      <FontAwesomeIcon
                                        icon={faComment}
                                ...
                                >
                                  <TouchableOpacity
                                    onPress={this._onClickedNewsfeedShare}
                                  >
                                    <HStack space={1}>
                                      <FontAwesomeIcon
                                        icon={faShare}
                      ...
                      >
                        ...
                        {this.commenting_in_progress != true && (
                          ...
                          >
                            ...
                            <TouchableOpacity
                              onPress={this._onClickedNewsfeedCommentSubmit}
                              style={{
                                width: 60,
                                height: 40,
                                padding: 5,
              ...
              this.orientation == "LANDSCAPE" && (
                ...
                >
                  <VideoPlayer
                    ...
                    onEnterFullscreen={this._onEnterFullscreen}
                    onExitFullscreen={this._onExitFullscreen}
                    timerText={COMMON_STYLES[i18n.language].regular}
                    style={{
                      width: "100%",
                      marginLeft: "auto",
                      marginRight: "auto",
  ...
  _onClickedNewsfeedLike = () => {
    ...
    if (this.newsfeed_detail.has_liked) {
      this.digiedModule
        .unlikeNewsfeed({ newsfeed_id: this.newsfeed_detail.id })
        .then((response) => {
          this.newsfeed_detail.is_liking_in_progress = false;
          this.newsfeed_detail.has_liked = false;
          this.newsfeed_detail.total_liked_count =
            Number(this.newsfeed_detail.total_liked_count) - 1;
      ...
      this.digiedModule
        .likeNewsfeed({ newsfeed_id: this.newsfeed_detail.id })
        .then((response) => {
          this.newsfeed_detail.is_liking_in_progress = false;
  ...
  _onClickedNewsfeedShare = () => {
    const { t, i18n } = this.props;
    let url =
      CommonConstants.weblink + "?_t=newsfeed&_id=" + this.newsfeed_detail.id;
    ...
    if (Platform.OS == "android") {
      ...
      Share.shareSingle(shareOptions)
        .then((response) => {
          ...
          try {
            this.digiedModule.shareNewsfeed({
              newsfeed_id: this.newsfeed_detail.id,
            });
          } catch (error) {
        ...
        .catch(() => {
          this.modalAlert = {
            ...
            description: t(
              "Unable to share the product. Please contact support team"
            ),
          };
          this.setState({
            forceRender: !this.state.forceRender,
  ...
  _onClickedNewsfeedCommentLike = (item) => () => {
    ...
    if (item.has_liked) {
      this.digiedModule
        .unlikeNewsfeedComment({
          newsfeed_id: item.newsfeed_id,
          newsfeed_comment_id: item.id,
        })
        .then((response) => {
          item.is_liking_in_progress = false;
...
function mapDispatchToProps(dispatch) {
  return {
    updateWishlistChanged: (wishlist_changed) =>
      dispatch({ type: "WISHLIST_CHANGED", payload: wishlist_changed }),
  };
}

export default withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(NewsfeedDetailScreen)
);
