...
class HomeScreen extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      ...
      this.state.language_changed &&
      this.state.language_changed != this.props.i18n.language
    ) {
      this.load();
      return false;
    }
  ...
  load = () => {
    ...
    this.digiedModule
      .getCourseCategory({ page: "all", include_home_banner_setting: 1 })
      .then((response) => {
        _.each(response.data.data, (data, index) => {
          ...
          if (
            i18n.language != "en" &&
            data.course_category_name_l10n &&
            data.course_category_name_l10n[i18n.language]
          ) {
          ...
          if (response.data.home_banner_setting.banner_image_1) {
            this.banners.push({
              image:
                CommonConstants.storage_endpoint +
                "/" +
                response.data.home_banner_setting.banner_image_1,
            });
          }
          ...
          if (response.data.home_banner_setting.banner_image_2) {
            this.banners.push({
              image:
                CommonConstants.storage_endpoint +
                "/" +
                response.data.home_banner_setting.banner_image_2,
            });
          }
          ...
          if (response.data.home_banner_setting.banner_image_3) {
            this.banners.push({
              image:
                CommonConstants.storage_endpoint +
                "/" +
                response.data.home_banner_setting.banner_image_3,
            });
          }
          ...
          if (response.data.home_banner_setting.banner_image_4) {
            this.banners.push({
              image:
                CommonConstants.storage_endpoint +
                "/" +
                response.data.home_banner_setting.banner_image_4,
            });
          }
          ...
          if (response.data.home_banner_setting.banner_image_5) {
            this.banners.push({
              image:
                CommonConstants.storage_endpoint +
                "/" +
                response.data.home_banner_setting.banner_image_5,
            });
          }
        }
  ...
  renderGroup = ({ item, index }) => {
    ...
    renderItem = (
      <TouchableOpacity
        onPress={this._onClickedGroup(item)}
        disabled={item.is_coming_soon == 1}
      >
        <AnimatableBox
          style={{
            // height: 240,
            ...
            <Text
              style={[
                COMMON_STYLES[i18n.language].bold,
                {
                  ...
                  marginRight: 2,
                  lineHeight:
                    Platform.OS == "ios" ? 30 : i18n.language == "mm" ? 28 : 26,
                },
              ]}
              numberOfLines={2}
            >
