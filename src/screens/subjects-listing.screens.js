...
class SubjectsListingScreen extends React.PureComponent {
  ...
  load = (forceRefresh) => {
    ...
    this.digiedModule
      .getSubjectsByCourse({ page: this.page, per_page: 5, course_id: item.id })
      .then((response) => {
        ...
        _.each(response.data.data, (data, index) => {
          ...
          if (
            i18n.language != "en" &&
            data.subject_name_l10n &&
            data.subject_name_l10n[i18n.language]
          ) {
          ...
          if (data.course) {
            ...
            if (
              i18n.language != "en" &&
              data.course.course_name_l10n &&
              data.course.course_name_l10n[i18n.language]
            ) {
    ...
    renderItem = (
      <TouchableOpacity onPress={this._onClickedSubject(item)}>
        ...
        >
          ...
          >
            <Text style={[COMMON_STYLES[i18n.language].bold]} numberOfLines={2}>
              {item.name}
            </Text>
          </Box>
        </Box>
...
export default withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(SubjectsListingScreen)
);
